...
public abstract class SerialControl {
    ...
    //当为0时，就在收到数据后会直接回调
    //接收的一组粘包数据会出现超时的情况
    public void init(String name,int speed, int databits, int parity, int stopbits, int flow_ctrl,int mdelay) { //一次性最大获取10k，在获取数据后，等待mdelay后，如果未在收到数据就回调数据
        File mFile = new File(name);
        mThreadRunning = true;
        send_wait_time = mdelay;
        if (mSerialPort == null) {
            mSerialPort = new SerialPort(mFile,speed, databits, parity, stopbits, flow_ctrl);
            uartWaitThread = new Thread(new Runnable() {
                @Override
                public void run() {
                    int len = 0;
